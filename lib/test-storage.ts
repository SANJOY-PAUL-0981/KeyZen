...
export function readStoredTestMode(): TestMode | undefined {
  ...
  if (!raw || !(VALID_TEST_MODES as readonly string[]).includes(raw)) return undefined;
...
export function readStoredQuoteLength(): QuoteLength | undefined {
  ...
  if (!raw || !(VALID_QUOTE_LENGTHS as readonly string[]).includes(raw)) return undefined;
...
export function readStoredDifficulty(): Difficulty | undefined {
  ...
  if (!raw || !(VALID_DIFFICULTIES as readonly string[]).includes(raw)) return undefined;
